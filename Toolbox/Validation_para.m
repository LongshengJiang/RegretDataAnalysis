...
% theory
    ...
    % Load indifferent points for ploting the q function. 
...
% -------------------------------------------------------------------------
   ...
   plot(Q_FunctionFit, Q_x1_RTx, Q_y1_RTx);
    ...
    % Calculate the term Q ( C(0) - C(x_H_star)) as Q_0H
    Q_0H_Vec_RTx =  Q_FunctionFit( -c_H_Vec );
    % Calculate the term Q ( C(x_R) - C(x_H_star)) as Q_0H
    Q_RH_Vec_RTx = Q_FunctionFit( c_R_Vec - c_H_Vec );
    
    % Compute the probability weights for RTx
    Pi_Prob_Vec_RTx = w_FunctionFit( Prob_Vec );
