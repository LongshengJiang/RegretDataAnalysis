function ShowMembership_EquallyLiked(filename, OptimalThetaVec)
% ShowMembership_EquallyLiked(filename, OptimalThetaVec) visualizes the
% real membership function of label 'equally liked'. It also visualizes the 
% classification performance in the training data. 
% Inputs:
...
% For example,
%       ShowMembership_EquallyLiked('Sample', [0.0146 0.3914 0.5096 0.6095 0.9967])
%       shows the membership function of label 'equally liked' of the sample dataset. 
...
% Plots of Indifferent
    ...
    title('Membership function of Indifference (RTx)');
    xlabel('e_Q');
    ylabel('\mu_I');
...
subplot(2,1,2);
    ...
    % Plot all the points where the choice is 'R'    
    plot(e_EV_Vec(Indx_R), Indif_Vec(Indx_R)+0.05, 'rs', 'Markerfacecolor', 'g');
    hold on;
    
    % Plot all the points where the choice is 'H'
    plot(e_EV_Vec(Indx_H), Indif_Vec(Indx_H)-0.05, 'bo', 'Markerfacecolor', 'g');

    % Plot all the points where the choice is 'I'
    plot(e_EV_Vec(Indx_I), Indif_Vec(Indx_I), 'g^', 'Markerfacecolor', 'g');
